...
export function UserAvatar({size=32}){
    ...
    return <span type="button" className="d-flex align-items-center" data-menu='login_modal'  onClick={(e) => openOffCanvas(e)}/* data-bs-target="#user_menu_drawer" */ /* data-bs-toggle={'modal'} */>
  ...
  } </div> : <>{sessionUser ? <Avatar width={size ?? 30} rounded src={sessionUser.image}  className="gx-size-40 gx-pointer" alt=""/> : <i className="far fa-user" style={{fontSize: size/2}} data-menu='login_modal' onClick={(e) => openOffCanvas(e)}/>}</>}</>}</Client>
     </span>
  }

...
export function UserAvatar({size=32}){
  ...
  } </div> : <>{user ? <Avatar width={size ?? 30} rounded src={sessionUser.image}  className="gx-size-40 gx-pointer" alt=""/> : <i className="far fa-user" style={{fontSize: size - 9}} data-menu='login_modal' onClick={(e) => openOffCanvas(e)}/>}</>}</>}</Client>
     </span>
  }

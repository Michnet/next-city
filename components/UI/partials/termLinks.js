...
export const TermIconBox = ({item, listingType, flipped=false, externalTitle=false, shadowy=true, exClass='', width='100px', height='100px'}) => {
    ...
    return <div><div key={id} className={`icon_bound box_icon  text-center overflow-hidden position-relative ${flipped ? '_flipped' : '_def'} ${exClass} ${shadowy ? 'shadow' : ''}`} style={{background: `${flipped ? color?.length > 0 ? color : 'var(--highlight)' : 'var(--bgTheme)'}`}}>
                    ...
                    {!externalTitle && <div className={`_title text-right ${flipped ? 'p-3' : 'px-3 pb-2'} d-flex align-items-end position-relative`} style={{height: height, width: width}}><Link onClick={() => closeMenus()} href={linker}  className={`${flipped ? 'color-white text-13 opacity-70' : 'color-theme text-10 opacity-80'} smLine fw-600 truncate-3  lh-12 heady`}>{cleanHtml(name).replace(' and ', ' & ')}</Link></div>}
                </div>
                {externalTitle && <div className={`_title text-center py-2 d-flex align-items-end position-relative opacity-60`} style={{ width: width}}><Link onClick={() => closeMenus()} href={linker}  className={`${flipped ? 'color-white text-13 opacity-70' : 'color-theme text-12'} smLine fw-600 truncate-2  lh-1 heady`}>{cleanHtml(name).replace(' and ', ' & ')}</Link></div>}
                </div>
}

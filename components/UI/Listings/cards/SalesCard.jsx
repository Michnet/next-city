...
const SalesCard = ({listing, width=320, dataAos, height='auto', exClass='', truncate=2, contentExClass='', titleSize=18}) => {
    ...
    return (<>
                    <div data-aos={dataAos} className={`listing_card bg-theme justify-center card card-style sale_card position-relative ${exClass}`} style={{height: height, width: width}}>
                    <div className={`img_div bg-cover position-absolute top-0 left-0 h-100`} style={{backgroundImage:`url("${xtra_large_thumb}")`}}>
                    {/* <div className="card-top p-3 flex-column d-flex align-items-end gap-1">
                            ...
                            <div className="sale_content bg-theme-transparent position-relative z-2 w-auto me-3 shadow">
                                <div className="d-grid overflow-hidden px-0 content_box">
                        <div className={`d-flex flex-column p-2 ps-3 _content ${contentExClass}`}>
                            <div className="title_content mb-1">
                            
                            <Link href={`/${type}s/${slug}`}><h1 className={`smLine font-${titleSize} font-800 truncate`}>{cleanHtml(title.rendered)}</h1></Link>
                        ...
                        {max_discount && <div className='discount_label'>

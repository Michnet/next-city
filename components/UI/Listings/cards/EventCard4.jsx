...
const EventCard4 = ({listing, width=260, height=200, exClass=''}) => {
    let {id, title, address, short_desc, event_date, page_views, rating, acf, locations, level, ticket_min_price_html, xtra_large_thumb, gallery, slug, thumbnail, type} = listing;
    return (<>
        <div className={`listing_card card card-style event_card_4 p-3 m-0 ${exClass}`} style={{width: width}}>
            <div className="d-flex align-items-center">
                ...
                {event_date && event_date[0] ? <span className="color-highlight font-300 d-block pt-0 text-uppercase font-10"><DateViewString date={event_date[0].start} format={'DD MMMM'}/></span> : <></>}
                    <Link href={`/${type}s/${slug}`}><strong className="color-theme font-15 d-block mt-n2 truncate-2 smLine my-1">{cleanHtml(title.rendered)}</strong></Link>

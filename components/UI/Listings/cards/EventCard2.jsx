    ...
    <div className={`card card-style`} style={{width: width}}>
            <div className="card shadow-l mb-0 bg-18" style={{width: 'inherit', height:height, backgroundImage: `url('${xtra_large_thumb}')`}}>
                <div className="card-bottom ms-3">
                    <p className="color-white font-10 opacity-80 mb-n1"><i className="color-highlight far fa-calendar"></i> <DateViewString date={event_date[0].start} format={'MMMM D'}/> <i className="color-highlight ms-3 far fa-clock"></i> <DateViewString date={event_date[0].start} format={'hh:mm A'}/> </p>
                    <p className="color-white font-10 opacity-80 mb-2"><i className="color-highlight fa fa-map-marker-alt"></i> {locations[0].name}</p>
                </div>
            ...
            <div className="d-flex flex-row flex-nowrap m-3 align-items-center gap-2">
                ...
                <Link href={`/events/${slug}`}><h3 className="truncate-2 text-15">{cleanHtml(title.rendered)}</h3></Link>
                    <p className="truncate font-11 mb-2 pb-1"><i className="fa fa-map-marker-alt me-2"></i>{address?.length > 0 ? address : locations[0].name}</p>
                </div>

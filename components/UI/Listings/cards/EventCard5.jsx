...
const EventCard5 = ({listing, width=300, height=170, exClass=''}) => {
    let {id, title, address, short_desc, event_date, page_views, thumbnail, rating, large_thumb, locations, level, ticket_min_price_html, xtra_large_thumb, category, slug, type} = listing;
    ...
    <div className={`listing_card card card-style round-medium shadow-huge top-30 m-0 event_card_5 ${exClass}`} style={{width: width, height: height}}>
            ...
            <div className="card-top mt-3 mx-3">
                ...
                {event_date?.length > 0 && event_date[0]?.start ? <p className="color-white font-11 opacity-80 mb-n1">
            ...
            <div className="card-bottom px-3 pb-2 row_flex gap-3 justify-between align-items-end">
                ...
                <Link href={`/${type}s/${slug}`} className="h-fit border-dark-light color-gray-dark text-nowrap  btn btn-s rounded-xl font-900 mt-2 text-uppercase font-11">Learn More</Link>
            </div>

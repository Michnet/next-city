...
function AuthorCard({author, setActiveKey, cover, exClass, listingId}) {
  ...
  if(author?.id){
    if(author.id == 1){
      ...
      <div style={{background: `url(${srcWithFallback(resizedImage(cover, 'medium_large'))})`}} className="d-block position-absolute z-1 h-100 bg-cover card-img-top position-relative">
      </div>

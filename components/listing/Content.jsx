...
function itContent(tabId, listing, gallery, community_id, colorHex, type, general_merchandise,setActiveKey,activeKey,color, faqs, isSample, tickets, id, chatCount, user, author_id){
  switch (tabId) {
      case 'gallery':
        console.log('gallery');
      const MegaGallery = dynamic(() => import("../UI/Galleries/MegaGallery"), {loading: () => <LoaderDualRingBoxed/>});
      return gallery?.length > 0 ? <LazyLoad><MegaGallery listing={listing} /* color={color} *//></LazyLoad> : 'empty';
      case 'reviews':
        const ListingReviews = dynamic(() => import("./reviews/Reviews"), {loading: () => <LoaderDualRingBoxed/>});
      ...
      case 'community':
      const ComponentActivity = dynamic(() => import("../UI/partials/ComponentActivity"), {loading: () => <LoaderDualRingBoxed/>});
      ...
      />
      case 'private-chat':
      const ProfileContact = dynamic(() => import("./partials/ProfileContact"), {loading: () => <LoaderDualRingBoxed/>});
      return <ProfileContact listing={listing} count={chatCount}/>
      case 'faqs':
      const FAQs = dynamic(() => import("../UI/FAQs"), {loading: () => <LoaderDualRingBoxed/>});
      ...
      case 'merchandise':
        const ListingProductsSimple = dynamic(() => import("./shop/ListingProductsSimple"), {loading: () => <LoaderDualRingBoxed/>});
      ...
      case 'tickets':
        const ListingTicketsSimple = dynamic(() => import("./shop/ListingProductsSimple"), {loading: () => <LoaderDualRingBoxed/>});
        ...
        <ListingTicketsSimple isSample ={isSample} listy ids={tickets} productType="booking" listingId = {id} relatedIds={tickets}/></div>
        :
...
function ContentConst({listing, colorHex, activeView, lMenu, activeKey, color, setActiveKey}) {
    ...
    const cacheditContent = useCallback(itContent, [id,activeKey]);

    

    //const cachedContent = useMemo(() => cacheditContent(id), [id] );
